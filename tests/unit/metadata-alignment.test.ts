...
describe('metadata alignment — v5.3.1', () => {
  it('AGENT.md references v5.3.1', () => {
    const content = readFile('AGENT.md');
    expect(content).toContain('v5.3.1');
  });
  ...
  it('NaSra.agent.md references v5.3.1', () => {
    const content = readFile('.github/agents/NaSra.agent.md');
    expect(content).toContain('v5.3.1');
  });

  it('CLAUDE.md header references v5.3.1', () => {
    const content = readFile('CLAUDE.md');
    const firstLine = content.split('\n')[0];
    expect(firstLine).toContain('v5.3.1');
  });
