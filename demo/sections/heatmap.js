...
export function createHeatmapSection() {
  ...
  section.innerHTML = `
    <style>
      ...
      #heatmap .mn-impact-row{display:grid;grid-template-columns:minmax(180px,1.4fr) minmax(160px,1fr) auto;gap:var(--space-md);align-items:center;padding:12px 0;border-bottom:1px solid var(--grigio-scuro)}
      #heatmap .mn-impact-track{height:10px;border-radius:999px;background:var(--grigio-scuro);overflow:hidden}#heatmap .mn-impact-track span{display:block;height:100%;border-radius:999px}
...
function buildImpactSimulation() {
  const cards = IMPACT.summary.map((card) => `<div class="mn-impact-summary__card" style="border-color:${card.border};color:${card.border === 'var(--rosso-corsa)' ? 'var(--rosso-corsa)' : 'var(--mn-accent)'}">${card.text}</div>`).join('');
  ...
  return `<div class="mn-card" style="padding:var(--space-xl)"><div class="mn-impact-summary">${cards}</div><div class="mn-impact-list">${rows}</div><div class="mn-impact-alert"><div class="mn-label" style="color:var(--mn-accent);margin-bottom:var(--space-xs)">Capacity gaps alert</div><div class="mn-body" style="margin:0">${IMPACT.alert}</div></div></div>`;
}
