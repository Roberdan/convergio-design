/**
 * <mn-tabs> — Web Component tabbed interface
 * Uses Maranello tabs CSS classes. Supports <mn-tab label="..."> children.
 *
 * @attr {number} active - Active tab index (default: 0)
 * @fires mn-tab-change  - {detail: {index, label}}
 * @version 1.4.0
 */
...
class MnTabs extends HTMLElement {
  ...
  _onKey(e, idx, total) {
    let next = idx;
    if (e.key === 'ArrowRight') { next = (idx + 1) % total; e.preventDefault(); }
    else if (e.key === 'ArrowLeft') { next = (idx - 1 + total) % total; e.preventDefault(); }
    else return;
    this._select(next);
