/**
 * <mn-command-palette> — Web Component Cmd+K command palette
 * Filterable command list with keyboard navigation. Standalone WC version
 * of Maranello.commandPalette.
 *
 * @attr {string} items       - JSON array [{text, icon?, shortcut?, group?}]
 * @attr {string} placeholder - Search input placeholder
 * @fires mn-select - {detail: {item}}
 * @version 1.4.0
 */
...
class MnCommandPalette extends HTMLElement {
  ...
  _parseItems() {
    try { this._items = JSON.parse(this.getAttribute('items') || '[]'); }
    catch { this._items = []; }
    this._filter();
  }
  ...
  _filter() {
    ...
    const filtered = q
      ? this._items.filter((it) => (it.text || '').toLowerCase().includes(q))
      : this._items;
